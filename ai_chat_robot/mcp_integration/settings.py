...
MCP_LOCAL_ENABLED = _env_bool("MCP_LOCAL_ENABLED", True)
MCP_LOCAL_SERVER_NAME = os.getenv("MCP_LOCAL_SERVER_NAME", "ecommerce-local")
...
MCP_LOCAL_APPROVAL_TOOLS = tuple(
    s.strip()
    for s in os.getenv("MCP_LOCAL_APPROVAL_TOOLS", "get_order_status").split(",")
    if s.strip()
)

# --- 托管式 MCP（公开远程，模型直连；需 Responses 兼容提供商）---
MCP_HOSTED_ENABLED = _env_bool("MCP_HOSTED_ENABLED", False)
MCP_HOSTED_SERVER_LABEL = os.getenv("MCP_HOSTED_SERVER_LABEL", "ecommerce-hosted")

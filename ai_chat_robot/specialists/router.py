...
def _router_instructions() -> str:
    lines = [
        ...
        "- **知识库 / RAG / 项目文档 / 学习笔记类问答** → 进入 ②，优先派 document_specialist。",
        "- 需要读工作区文件、导出 csv/xlsx/docx、长文写作、数据分析 → 进入 ②。",
        "",
        "### ② 派发（Action）",
        "- **信息不足**（如缺作文主题、课程要求、数据范围）→ 先用中文与用户对齐 1～3 个关键问题，"
        "可自然说明将交给哪位专员处理；**本轮不 handoff**。",
        "- **信息已足够** → 调用 transfer_to_* handoff；专员执行期间不要向用户直播中间过程。",
        "- document：读工作区文件 / 表格导出 / **知识库检索（retrieve_knowledge_base，BM25+向量）**",
        "- 知识库类问题（如「根据知识库解释 ReAct」「项目文档怎么说」）→ **必须** transfer_to_document_specialist",
